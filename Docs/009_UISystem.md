...
> 2026-06-29 BGM Revision：MainMenu 与 Game 场景各自放置一个本地 BGM AudioSource 音乐机，使用场景内引用的 AudioClip，`Play On Awake = true`、`Loop = true`。不使用跨场景持久化对象；MainMenu 使用 `Assets/6月29日 (2).mp3`，Game 使用 `Assets/6月29日 (4).mp3`。
...
- BGM / 音效播放 → 场景本地 AudioSource 或各系统直接调用 AudioSource，不走 UISystem

---

## Scene BGM

- `MainMenu.unity`：根对象 `MainMenuMusicMachine`，挂载 `AudioSource`，播放 `Assets/6月29日 (2).mp3`。
- `Game.unity`：根对象 `GameMusicMachine`，挂载 `AudioSource`，播放 `Assets/6月29日 (4).mp3`。
- 两个 `AudioSource` 均设置 `Play On Awake = true`、`Loop = true`、`Volume = 1`。
- 不使用 `DontDestroyOnLoad`，避免主菜单音乐进入 Game 场景后继续叠播。
