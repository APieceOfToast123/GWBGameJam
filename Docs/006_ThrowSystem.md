...
- 生成面包投射物 Prefab 并驱动沿球道前进的轻抛物线动画
...
- 面包从 ThrowOrigin 发出，先贴到目标球道底部点位，再沿目标球道中心线飞向目标点；飞行时间 = ThrowDuration（固定时长，与距离无关）。
...
面包飞行轨迹由参数 t ∈ [0, 1] 驱动，分为两段：

```
if t < 0.25:
    pathPosition = Lerp(startPos, laneEntryPos, t / 0.25)
else:
    pathPosition = Lerp(laneEntryPos, endPos, (t - 0.25) / 0.75)

position(t) = pathPosition + Vector2.up * PeakHeight * 4 * t * (1 - t)
```

- `startPos`：桌子/手部投掷起点（ThrowOrigin，Inspector 配置）
- `laneEntryPos`：目标 lane 靠近桌子的最后一个 waypoint
- `endPos`：目标点世界坐标（捕获时确定，动画期间不再更新）
- `PeakHeight`：弧顶高度（世界单位，ThrowConfig 配置）
- `t`：= elapsed / ThrowDuration，线性增长

特性：前段让面包从桌子位置接入目标 lane，后段保持沿 lane 中心线向前飞；轻抛物线只提供高度感，不改变命中判定。
...
```
endPos = LaneSystem.GetWaypoint(laneIndex, 0)
```

取该球道远端点位，使空击时也能清楚看到面包沿球道向前飞行。
...
- [ ] Given 面包飞行中，When 在 Scene 视图观察，Then 面包先接入目标 lane 底部，再沿该 lane 中心线向前运动，并保留轻微弧度
