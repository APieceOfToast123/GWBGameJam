...
**[Tech] Unity MCP 切换为 CoplayDev/unity-mcp v9.7.3，并使用本地 HTTP 传输**
原因：旧 GameLovers MCP 的 PackageCache 入口已不存在，导致 Codex 无法启动服务；CoplayDev 官方当前支持 Unity 2021.3+、Codex TOML 配置和 `http://localhost:8080/mcp` 默认传输。锁定稳定标签避免 `main` 后续变化造成不可复现安装。
